...
/**
 * Gera textura circular suave para partículas estelares de fundo
 */
function createStarTexture() {
  ...
  grad.addColorStop(0.2, 'rgba(255, 245, 220, 0.85)');
  grad.addColorStop(0.5, 'rgba(180, 215, 255, 0.3)');
...
export default function OrbitalScene({ onSelect }) {
  ...
  useEffect(() => {
    ...
    controls.enableZoom = false; // Scroll da página 100% livre
    controls.minPolarAngle = 0.1;
    controls.maxPolarAngle = Math.PI / 2 + 0.12;

    // Iluminação cinematográfica do Sistema Solar
    const ambientLight = new THREE.AmbientLight(0xdceeff, 1.2);
    ...
    // ── CAMPO ESTELAR CÓSMICO (SEM GALÁXIA POLUINDO A ÓRBITA) ──
    const starCount = 800;
    ...
    // ── CRIAR O SOL E OS 8 PLANETAS COM SUAS ÓRBITAS ──
  ...
  return (
    <div className="w-full flex flex-col items-center relative">
      {/* ── PALCO 3D ABERTO E NATURAL NO FUNDO NORMAL DO ESPAÇO ── */}
      <div 
        className="w-full h-[480px] sm:h-[580px] lg:h-[680px] relative overflow-hidden bg-transparent"
      ...
      <div className="w-full max-w-4xl mt-3 flex items-center justify-center gap-1.5 sm:gap-2 flex-wrap px-2">
        {BODIES.map(body => (
          <button
            key={body.id}
            onClick={() => onSelect(body.id)}
